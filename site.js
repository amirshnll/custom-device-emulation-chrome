...
function render() {
 ...
 for (const d of filtered) {
  ...
  if (d.source && /^https?:\/\//i.test(d.source)) { const link=node('a','View source'); link.href=d.source; link.rel='noopener noreferrer'; detail.append(link,node('p',`Last source check: ${d.last_verified}`)); } else detail.append(node('p','Original source and verification date are unknown.'));
